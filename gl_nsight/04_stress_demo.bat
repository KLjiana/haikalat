...
echo Starting StressDemo with %INSTANCES% %SHAPE% instances...
echo Usage: 04_stress_demo.bat [triangle^|quad^|cube] [instance-count]
echo In Nsight Graphics, attach to the java.exe process whose window title starts with Stress.
call gradlew.bat runStressDemo -PstressShape=%SHAPE% -PstressInstances=%INSTANCES% --no-daemon --console=plain
