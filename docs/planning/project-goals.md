...
- 3D 管线：forward pipeline、scene/camera/object/light 模型、带输入校验和数量上限的基础光照、
  ...
  stable camera/shadow frustum、revision 驱动的静态 model/world-bounds cache、独立可复用 Render Queue，
  以及 FXAA/TAA/MSAA/none 后处理选择。
...
- Demo 证明：`LearnOpenGlDemo` 作为综合场景、阴影、后处理证明；`MinimalDemo` 验证最小窗口、
  ...
  验证布局、输入、文本、弹窗、滚动、虚拟化和 UI 渲染链路；`SceneScalabilityDemo` 与
  `GltfSceneScalabilityDemo` 分别验证程序化和共享 GPU 资产的 100/1k/10k traversal、裁剪、队列、
  command encoding 和状态切换。
