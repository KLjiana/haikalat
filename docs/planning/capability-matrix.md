...
| Instancing | 完整 | `InstancedMeshBatch`、`InstancedRenderer`、`CommandBuffer.drawInstancedBatch` | 三个 Demo | `CommandBufferTest`、`InstanceDataLayoutTest`、`RenderPipelineGlTest.instancedShadowOptInChangesPixelsAndBatchRemainsReusableNextFrame` | 实例阴影默认关闭；开启后 shadow/geometry 各执行一次安全 batch 生命周期并分别使用 ring slot/fence |
...
| HDR / ACES tone mapping | 完整 | `RenderSettings`、`ToneMappingMode`、`ToneMappingPass`、`RGBA16F` framebuffer | `LearnOpenGlDemo` | `RenderSettingsTest`、`ToneMappingPassTest`、`FramebufferDescriptorTest`、`RenderPipelineGlTest` | 默认保持 LDR；主 Demo 显式使用 ACES/exposure 1.0；暂不包含自动曝光与 Bloom |
