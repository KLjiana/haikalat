...
| UI Tween / Transition | 完整 | `subsystems/ui/animation`、`UiSystem.animations()` | `UiDemo`、`HaikalatShowcaseDemo` | `UiAnimationSystemTest`、`UiSystemTest`、`UiDemoGlTest`、`runUiIntegration`、`runShowcaseIntegration` | 支持 visual/layout 通道、linear/cubic/spring、delay、暂停/恢复、进度、取消/替换及确定性诊断快照；布局过渡要求单位和离散字段兼容，尚无 sequence/shared-element graph |
| CPU VFX 粒子、Ribbon 与 Decal | 完整 | `subsystems/vfx`、`subsystems/render3d/vfx/VfxRenderer` | `VfxDemo`、`HaikalatShowcaseDemo` | `EffectInstanceTest`、`VfxRendererGlTest`、`runVfxIntegration`、`runVfxPerformanceBaseline`、`runShowcaseIntegration` | `EffectAsset/EffectInstance` 资产实例分离；固定种子 CPU 模拟、有界容量/寿命、透明稳定排序、关闭顺序和 resize 已验证；当前 GL adapter 仍为逐 primitive draw |
...
- VFX 本地验收：默认 `test` 覆盖固定种子、容量、寿命、排序和资产关闭；`VfxRendererGlTest` 与 `runVfxIntegration` 验证三类 primitive、最终像素和 resize；`runVfxPerformanceBaseline` 输出 CPU/GPU/uniform payload 基线。
