...
- 新增三种不可变室外环境预设、风格化天空、主太阳绑定和最多 8 个球/盒局部雾体；
- 新增复用当前方向光 CSM 的有限距离体积积分、半分辨率独立历史、邻域裁剪和深度感知上采样，
  体积散射在线性 HDR、曝光和 Bloom 之前合成，UI 不受雾化；
- 新增 `runOutdoorEnvironmentDemo`、`runOutdoorEnvironmentIntegration`、稳定性与 benchmark 入口，
  明确 LDR、透明队列和 HDR VFX 组合限制；
- 修正 PBR IBL 漫反射的 `E / PI` 归一化，并补充室外环境指南和开发阶段性能记录。
- 体积历史改为独立 R16F 深度与上一帧 VP 重投影，补充高采样参考档、物理参考测试、
  五轮 1080p/4K 配对 benchmark，以及 3,600 帧/100 次切换/10 次失败注入专项证据。
