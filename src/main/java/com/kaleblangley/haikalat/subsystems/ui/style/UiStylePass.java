...
public final class UiStylePass {
    ...
    private UiNode previousFocused;
    ...
    public int resolve(UiDocument document) {
        ...
        int changed = resolveNode(document, document.root(), null, true,
                focused, previousFocused, false);
        changed += resolveNode(document, document.overlayRoot(), null, true,
                focused, previousFocused, false);
    ...
    private int resolveNode(UiDocument document, UiNode node, ComputedStyle inherited,
                            boolean root, UiNode focused, UiNode oldFocused,
                            boolean inheritedChanged) {
        boolean resolveSelf = inheritedChanged || node.isDirty(UiDirtyFlag.STYLE)
                || node == focused || node == oldFocused;
        ComputedStyle resolved = node.computedStyle();
        ...
        if (resolveSelf) {
            ...
            if (focused == node) states.add(StyleResolver.PseudoState.FOCUSED);
        ...
        for (UiNode child : node.children()) {
            count += resolveNode(document, child, resolved, false,
                    focused, oldFocused, changed);
        }
        return count;
    }
}
