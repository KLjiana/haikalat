...
final class GltfNodeDecoder {
    ...
    Result decode(List<Map<String, Object>> nodeDefinitions, int meshCount) {
        SceneChoice scene = selectScene();
        return new Result(scene.index(), scene.name(), scene.roots(),
                decodeNodes(nodeDefinitions, meshCount, scene.roots()));
    }
    ...
    private List<LoadedGltfScene.Node> decodeNodes(List<Map<String, Object>> definitions,
                                                   int meshCount, List<Integer> roots) {
        ...
        for (int index = 0; index < count; index++) {
            ...
            if (definition.containsKey("skin") || definition.containsKey("weights")) {
                throw fail(path, "skins and morph weights are not supported");
            }
            ...
            for (int child : childList) index(child, count, path + ".children");
            children.add(childList);
            locals[index] = localMatrix(definition, index);
        }
    ...
    private Matrix4f localMatrix(Map<String, Object> definition, int index) {
        String path = "nodes[" + index + "]";
        ...
        if (definition.containsKey("matrix")) {
            return new Matrix4f().set(floatArray(definition.get("matrix"), 16, path + ".matrix"));
        }
