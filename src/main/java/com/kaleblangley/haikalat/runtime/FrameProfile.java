...
public record FrameProfile(long cpuFrameNanos, List<PassProfile> passes) {
    ...
    public FrameProfile {
        if (cpuFrameNanos < 0L) {
            throw new IllegalArgumentException("cpuFrameNanos must be non-negative");
        }
    ...
    public long totalGpuNanos() {
        long total = 0L;
        for (PassProfile pass : passes) {
            total += pass.gpuNanos();
        }
        return total;
    }

    public double cpuFrameMillis() {
        return cpuFrameNanos / 1_000_000.0;
    }

    public double totalGpuMillis() {
        return totalGpuNanos() / 1_000_000.0;
    }
}
