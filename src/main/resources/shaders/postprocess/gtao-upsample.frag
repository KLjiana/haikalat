...
void main() {
    ...
    vec2 halfPixel = vTexCoord * uHalfExtent;
    vec2 texel = 1.0 / max(uHalfExtent, vec2(1.0));
    float sum = 0.0;
    float weights = 0.0;
    for (int x = 0; x <= 1; x++) for (int y = 0; y <= 1; y++) {
        vec2 uv = clamp((floor(halfPixel) + vec2(x, y) + 0.5) * texel,
                vec2(0.0), vec2(1.0));
        ...
        float edgeWeight = normalWeight(centerNormal,
                reconstructNormal(uv, tapDepth, tapView));
        float weight = max(edgeWeight, 1.0e-3);
        sum += texture(uInput, uv).r * weight;
        weights += weight;
    }
